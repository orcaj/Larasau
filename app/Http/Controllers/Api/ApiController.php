...
class ApiController extends Controller
{
    ...
    private function userInfo($id){
        ...
        $sell['today']  = Transaction::whereIn('content_id', $contents)->where('mode','deliver')->where('create_at','>',$day)->count();
        $sell['month']  = Transaction::whereIn('content_id', $contents)->where('mode','deliver')->where('create_at','>',$month)->count();
    ...
    public function functionIndex(){
        ...
        foreach ($result['blogPosts'] as $post){
            $data['content']['news'][] = [
                'title'     => $post->title,
                'date'      => date('Y F d', $post->create_at),
        ...
        foreach ($result['articlePosts'] as $article){
            $data['content']['article'][] = [
                'title'     => $article->title,
                'date'      => date('Y F d', $article->create_at),
    ...
    public function product($id,Request $request){
        ...
        if(isset($User)){
            ...
            foreach ($supports as $s){
                ...
                $support[] = [
                    ...
                    'comment'   => $s->comment,
                    'type'      => $type,
                    'date'      => date('Y F,d H:i', $s->create_at)
                ];
            }
        }
        ...
        $data    = [
            ...
            'size'          => isset($MB)?$MB:0,
            'date'          => date('Y-m-d', $content->create_at),
            'parts'         => $parts,
    ...
    public function userProfile(Request $request)
    {
        ...
        foreach ($videos as $viid) {
            ...
            $data['videos'][] = [
                'id'        => $viid->id,
                'title'     => $viid->title,
                'date'      => date('Y F d', $viid->create_at),
                'thumbnail' => checkUrl($meta['thumbnail']),
                'currency'  => $currency,
                'duration'  => isset($meta['duration'])?convertToHoursMins($meta['duration']):0,
    ...
    public function supportList(Request $request){
        ...
        foreach ($comments as $comment){
            $data['comments'][] = [
                ...
                'date'      => date('Y F d', $comment->create_at),
                'comment'   => $comment->comment
            ];
        }
        ...
        foreach ($supports as $support){
            $data['supports'][] = [
                ...
                'date'      => date('Y F d', $support->create_at),
        ...
        foreach ($tickets as $index=>$ticket){
            $data['tickets'][] = [
                ...
                'mode'      => $ticket->mode,
                'date'      => date('Y F d', $ticket->create_at),
            ];
            foreach ($ticket->messages as $message){
                $data['tickets'][$index]['messages'][] = [
                    ...
                    'mode'  => $message->mode,
                    'date'  => date('Y F d', $message->create_at),
                    'view'  => $message->view
                ];
            }
        }
        ...
        foreach ($purchases as $item){
            if(isset($item->content)) {
                ...
                $data['purchases'][] = [
                    ...
                    'currency'  => $currency,
                    'date'      => date('Y F d | H:i', $item->create_at)
                ];
            }
        }
    ...
    public function walletVerify(Request $request){
        if(isset($request->gateway) && $request->gateway == 'paypal') {
            ...
            if (empty($request->PayerID) || empty($request->token)) {
                \Session::put('error', 'Payment failed');
                return view('app.wallet',['mode'=>'failed']);
            }
            $payment = Payment::get($payment_id, $this->_api_context);
            $execution = new PaymentExecution();
            $execution->setPayerId($request->PayerID);
            ...
            if ($result->getState() == 'approved') {
                ...
                return view('app.wallet',['mode'=>'successfully']);
            }
        }
